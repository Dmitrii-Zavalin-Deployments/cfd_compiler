...
def valid_container() -> SovereignContainer:
    ...
    and element size parameters initialized with canonical state schemas.
    """
...
def test_assembly_step_success(
    assembly_step: AssemblyStep, valid_container: SovereignContainer, caplog
):
    """
    ...
    """
    expected_status = dummy_out()["status"]
    with caplog.at_level(logging.INFO):
        assembly_step.execute(valid_container)

    # Validate container updates
    assert valid_container.compiled_cells_count == 750
    assert valid_container.status == expected_status

    # Validate informational logging
    assert "Executing AssemblyStep..." in caplog.text
    assert (
        "CFD Compilation finished successfully. Discretized domain (5x10x15)."
        in caplog.text
    )
    assert "Total compiled cells: 750" in caplog.text


def test_assembly_step_clamping_to_minimum_cells(
    assembly_step: AssemblyStep, valid_container: SovereignContainer
):
    """
    ...
    assert valid_container.status == dummy_out()["status"]


# --- TESTS: CONSTITUTION VIOLATIONS & ERROR HANDLING ---


def test_assembly_step_missing_bounding_box(
    assembly_step: AssemblyStep, valid_container: SovereignContainer
):
    """
    Verifies that a ValueError is raised when bounding_box is None.
    """
...
def test_assembly_step_invalid_max_element_size(
    ...
    """
    Verifies that a ValueError is raised when max_element_size is None or <= 0.
    """
...
def test_assembly_step_degenerate_geometry(
    assembly_step: AssemblyStep,
    valid_container: SovereignContainer,
    degenerate_box: tuple,
):
    """
    Verifies that a ValueError is raised when any spatial dimension has zero thickness.
    """
...
def test_assembly_step_slots_enforcement(assembly_step: AssemblyStep):
    ...
    with pytest.raises(
        AttributeError,
        match="'AssemblyStep' object has no attribute 'dynamic_attr'",
    ):
